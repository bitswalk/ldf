    ...
    <section class="space-y-6">
      ...
      <header class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-bold">{t("build.detail.title")}</h1>
          ...
          <div class="flex items-center gap-3">
            <Show when={isBuildActive(build()!.status)}>
              <button
                ...
                class="px-4 py-2 rounded-md border border-border hover:bg-muted transition-colors disabled:opacity-50 flex items-center gap-2"
              >
                <Show when={actionLoading()} fallback={<Icon name="x" size="sm" />}>
                  <Spinner size="sm" />
                </Show>
            ...
            <Show when={canRetryBuild(build()!.status)}>
              <button
                onClick={handleRetry}
                disabled={actionLoading()}
                class="px-4 py-2 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center gap-2"
              >
                <Show when={actionLoading()} fallback={<Icon name="arrow-clockwise" size="sm" />}>
                  <Spinner size="sm" />
                </Show>
      ...
      <Show when={error()}>
        <div class="text-center py-12">
          <Icon name="warning" size="xl" class="mx-auto mb-4 text-destructive" />
            ...
            <Card header={{ title: t("build.detail.info.title") }}>
              <div class="space-y-4">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="font-medium">{build()!.target_arch}</span>
                </div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="font-medium">{build()!.progress_percent}%</span>
                </div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-sm">{formatDate(build()!.created_at)}</span>
                </div>
                  ...
                  <For each={build()!.stages}>
                    {(stage) => (
                      <button
                        ...
                        onClick={() => {
                          setSelectedStage(
                            selectedStage() === stage.name ? null : stage.name
                          );
                          fetchLogs();
                        }}
                      >
                        ...
                        <div class="flex-1">
                          <div class="font-medium">
                            {getStageDisplayText(stage.name as BuildStageName)}
                          </div>
